...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        recruiters = orm.User.objects.filter(recruiter_for__isnull=False)
        students = orm.User.objects.filter(recruiter_for__isnull=True)
        for recruiter in recruiters:
            extended_user = orm.Recruiter(user_ptr_id=recruiter.id)
            extended_user.__dict__.update(recruiter.__dict__)
            extended_user.company = recruiter.recruiter_for
            extended_user.save()
        for student in students:
            extended_user = orm.Student(user_ptr_id=student.id)
            extended_user.__dict__.update(student.__dict__)
            extended_user.save()
