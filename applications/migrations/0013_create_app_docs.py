...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for app in orm['applications.Application'].objects.all():
            # 1. create a new ApplicationDocument
            document = orm['documents.Document'].objects.filter(document_type=1, user=app.user).order_by('-timestamp')
            if document:
                new_doc = orm.ApplicationDocument(application=app, document=document[0])
                new_doc.save()
